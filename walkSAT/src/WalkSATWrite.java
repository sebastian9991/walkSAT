...
public class WalkSATWrite {
     ...
     */
    public List<Object> walkSAT(Set<Clause> clauses, double p, int maxFlips) {
        List<Object> returned_list = new ArrayList<>();
            ...
            for (int i = 0; i < maxFlips || maxFlips < 0; i++) {
                ...
                if (System.currentTimeMillis() < end) {
                    if (model.satisfies(clauses)) {
                        returned_list.add(model);
                        returned_list.add(number_of_flips);
     ...
     */
    public WalkSATWrite(Random random) {
        this.random = random;
    }
